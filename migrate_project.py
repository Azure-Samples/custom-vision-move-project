...
def migrate_tags(src_trainer, dest_trainer, project_id, dest_project_id):
    ...
    for tag in src_trainer.get_tags(project_id):
        ...
        created_tags[tag.id] = dest_trainer.create_tag(
            dest_project_id, tag.name, description=tag.description, type=tag.type).id
    return created_tags
...
def migrate_images(src_trainer, dest_trainer, project_id, dest_project_id, created_tags):
    ...
    while(count > 0):
        ...
        images = src_trainer.get_tagged_images(
            project_id, take=count_to_migrate, skip=migrated)
        images_to_upload = []
        for i in images:
            ...
            if i.regions:
                regions = []
                for r in i.regions:
                    print("Found region:", r.region_id, r.tag_id,
                          r.left, r.top, r.width, r.height)
                    regions.append(Region(
                        tag_id=created_tags[r.tag_id], left=r.left, top=r.top, width=r.width, height=r.height))
                entry = ImageUrlCreateEntry(
                    url=i.original_image_uri, regions=regions)
            else:
                tag_ids = []
                for t in i.tags:
                ...
                entry = ImageUrlCreateEntry(
                    url=i.original_image_uri, tag_ids=tag_ids)
    ...
    while(count > 0):
        count_to_migrate = min(count, 50)
        print("Getting", count_to_migrate, "images")
        images = src_trainer.get_untagged_images(
            project_id, take=count_to_migrate, skip=migrated)
        images_to_upload = []
        for i in images:
            print("Migrating", i.id, i.original_image_uri)
...
def migrate_project(src_trainer, dest_trainer, project_id):
    ...
    print("\tTarget Export Platforms:",
          src_project.settings.target_export_platforms)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("-p", "--project", action="store", type=str,
                            help="Source project ID", dest="project_id", default=None)
    arg_parser.add_argument("-s", "--src", action="store", type=str,
                            help="Source Training-Key", dest="source_training_key", default=None)
    arg_parser.add_argument("-se", "--src_endpoint", action="store", type=str, help="Source Endpoint",
                            dest="source_endpoint", default="https://southcentralus.api.cognitive.microsoft.com")
    arg_parser.add_argument("-d", "--dest", action="store", type=str,
                            help="Destination Training-Key", dest="destination_training_key", default=None)
    arg_parser.add_argument("-de", "--dest_endpoint", action="store", type=str, help="Destination Endpoint",
                            dest="destination_endpoint", default="https://southcentralus.api.cognitive.microsoft.com")
    ...
    # Client for Source
    src_trainer = CustomVisionTrainingClient(
        args.source_training_key, endpoint=args.source_endpoint)

    # Client for Destination
    dest_trainer = CustomVisionTrainingClient(
        args.destination_training_key, endpoint=args.destination_endpoint)

    destination_project = migrate_project(
        src_trainer, dest_trainer, args.project_id)
    tags = migrate_tags(src_trainer, dest_trainer,
                        args.project_id, destination_project.id)
    source_images = migrate_images(
        src_trainer, dest_trainer, args.project_id, destination_project.id, tags)
